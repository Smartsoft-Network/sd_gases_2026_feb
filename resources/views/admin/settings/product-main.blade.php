...
@section('title', 'Product Main Page Settings')

@section('content')
<section class="section">
    <div class="section-body">
        <form action="{{ route('admin.settings.product-main.update') }}" method="POST">
            @csrf
            <div class="row">
                <div class="col-12 text-center mb-4 mt-2">
                    <div class="py-4 bg-light rounded shadow-sm">
                        <h2 class="font-weight-bold text-dark mb-2">Product Main Page Settings</h2>
                        <p class="text-muted mb-0">Configure the content for the main products page.</p>
                    </div>
                </div>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="hero_subtitle" class="form-control" rows="3" required>{{ $productMainData['hero_subtitle'] ?? '' }}</textarea>
                            </div>

                            {{-- Features Bar --}}
                            <div class="form-group">
                                <label>Features Bar (Public Products Page)</label>
                            ...
                            <div class="form-group mt-5 pt-4 border-top">
                                <label>Category Section - Browse Text</label>
                                <input type="text" name="category_section_browse" class="form-control" value="{{ $productMainData['category_section_browse'] ?? '' }}" required>
                            </div>
                            <div class="form-group">
                                <label>Category Section - Title</label>
                                <input type="text" name="category_section_title" class="form-control" value="{{ $productMainData['category_section_title'] ?? '' }}" required>
                            </div>
                            <div class="form-group">
                                <label>Category Section - Description</label>
                                <textarea name="category_section_desc" class="form-control" rows="3" required>{{ $productMainData['category_section_desc'] ?? '' }}</textarea>
                            </div>
...
@push('scripts')
<script>
    let featureIndex = {{ count($productMainData['features'] ?? []) }};
